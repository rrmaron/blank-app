...
if rating_result and len(real_games) >= 5:
    st.write(f"Performance/Initial Rating: {rating_result['Rp']} = Average opponent rating + dp (dp is usually negative, so subtracted) = {rating_result['avg']} + {rating_result['dp']:-}  ")
